function [total_energy,cons] = simulation_2quads_visual(trajhandle, controlhandle,trajhandle2, controlhandle2,Pb_i,Vb,t5)
% function [total_energy,cons] = simulation_2_agents(trajhandle, controlhandle,Pb_i,Vb,t5)
cn=1;
tic
time_cap=[];
cons=[];%zeros(2,1);
...
des_pos2=[des_pos2;desired_state2.pos']; sepr=norm(des_pos(end,:)-des_pos2(end,:));
...
%     set(h_title, 'String', sprintf('separation: %4.2f',sepr))
if msep>sepr
    msep=sepr;
end
h_title = title(sprintf('separation: %4.2f m\nminimum separation: %4.2f m', sepr,msep));
